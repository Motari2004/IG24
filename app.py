...
class InstagramBot:
    ...
    async def search_hashtag(self, hashtag):
        ...
        try:
            ...
            self.web_log("LOG: Waiting for div._aagu containers to render...")
            await self.page.wait_for_selector('div._aagu', timeout=70000, state="visible")
            ...
            self.web_log(f"DATA: Successfully extracted {len(unique_urls)} posts via _aagu selector.")
    ...
    async def process_post(self, post_url):
        ...
        try:
            ...
            try:
                ...
                self.web_log("LOG: Content render slow, proceeding anyway...")
...
def serve_screenshot(filename):
    if not filename.startswith('follow_start_') and not filename.startswith('modal_fail_') and not filename.startswith('search_timeout_'):
        abort(403)
    try:
